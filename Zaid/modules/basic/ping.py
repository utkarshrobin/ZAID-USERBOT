...
async def pingme(client: Client, message: Message):
    ...
    try:
       ...
       pass
    await xx.edit("**20% Load hua h Utkarsh**")
    await xx.edit("**40% Ho gya Utkarsh**")
    await xx.edit("**60% Bs aane wala h **")
    await xx.edit("**80% ████████▒▒**")
    await xx.edit("**100% UTKARSH AA GYA **")
    end = datetime.now()
    duration = (end - start).microseconds / 1000
    await xx.edit(
        f"❏ **╰☞ 𝗣𝗢𝗡𝗚™╮**UTKARSH SB KI MAA CHODEGA** **\n"
        f"├• **╰☞** - `%sms`\n"
        f"├• **╰☞ -** `{uptime}` \n"
